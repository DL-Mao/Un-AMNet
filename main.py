# ------------------------------------------------------------------
# AMNet: A Simple Network for Image Anomaly Detection and Localization (https://openaccess.thecvf.com/content/CVPR2023/papers/Liu_SimpleNet_A_Simple_Network_for_Image_Anomaly_Detection_and_Localization_CVPR_2023_paper.pdf)
...
_DATASETS = {
    ...
    "code10": ["datasets.code10", "Code10Dataset"],
}
...
def run(
    ...
    for dataloader_count, dataloaders in enumerate(list_of_dataloaders):
        ...
        amnet_list = methods["get_amnet"](imagesize, device)

        models_dir = os.path.join(run_save_path, "models")
        os.makedirs(models_dir, exist_ok=True)
        for i, AMNet in enumerate(amnet_list):
            # torch.cuda.empty_cache()
            if AMNet.backbone.seed is not None:
                utils.fix_seeds(AMNet.backbone.seed, device)
            LOGGER.info(
                "Training models ({}/{})".format(i + 1, len(amnet_list))
            )
            # torch.cuda.empty_cache()

            AMNet.set_model_dir(os.path.join(models_dir, f"{i}"), dataset_name)
            if not test:
                segmentation_save_path = os.path.join(run_save_path, "segmentation_images", dataset_name, f"{i}")
                i_auroc, p_auroc, pro_auroc = AMNet.train(
                ...
                # i_auroc, p_auroc, pro_auroc =  AMNet.test(dataloaders["training"], dataloaders["testing"], save_segmentation_images)
...
@click.option("--use_barcode_discriminator", is_flag=True, default=False, help="Use barcode adaptive discriminator")
def net(
    backbone_names,
    ...
    def get_amnet(input_shape, device):
        amnets = []
        for backbone_name, layers_to_extract_from in zip(
            ...
            amnet_inst.load(
                ...
                mix_noise=mix_noise,
                use_barcode_discriminator=use_barcode_discriminator,
            )
            amnets.append(amnet_inst)
        return amnets

    return ("get_amnet", get_amnet)
