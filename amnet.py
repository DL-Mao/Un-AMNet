...
from simplified_barcode_discriminator import BarcodeDiscriminatorWrapper, AdaptiveLossFunction
...
class AMNet(torch.nn.Module):
    ...
    def load(
        ...
        pre_proj=0, # 1
        proj_layer_type=0,
        use_barcode_discriminator=False,  
        **kwargs,
    ):
        pid = os.getpid()
        def show_mem():
        ...
        self.use_barcode_discriminator = use_barcode_discriminator
        
        if use_barcode_discriminator:
            self.discriminator = BarcodeDiscriminatorWrapper(
                self.target_embed_dimension, 
                n_layers=dsc_layers, 
                hidden=dsc_hidden
            )
            self.adaptive_loss = AdaptiveLossFunction(margin=dsc_margin)
        else:
    ...
    def train(self, training_data, test_data, save_segmentation_images=False, save_path=None):

        
        state_dict = {}
        ckpt_path = os.path.join(self.ckpt_dir, "ckpt.pth")
        best_segmentations = None
        best_scores = None
        ...
        if os.path.exists(ckpt_path):
            ...
            if save_segmentation_images:
                if save_path is None:
                    save_path = os.path.join(self.ckpt_dir, "segmentation_images")
                self.save_segmentation_images(test_data, segmentations, scores, save_path)
        ...
        for i_mepoch in range(self.meta_epochs):
            ...
            if best_record is None:
                ...
                best_segmentations = np.array(segmentations).copy() if isinstance(segmentations, list) else segmentations.copy()
                best_scores = np.array(scores).copy() if isinstance(scores, list) else scores.copy()
        ...
        if save_segmentation_images:
            if save_path is None:
                save_path = os.path.join(self.ckpt_dir, "segmentation_images")
            if best_segmentations is not None and best_scores is not None:
                self.save_segmentation_images(test_data, best_segmentations, best_scores, save_path)
            else:
                self.save_segmentation_images(test_data, segmentations, scores, save_path)
        ...
        with tqdm.tqdm(total=self.gan_epochs) as pbar:
            for i_epoch in range(self.gan_epochs):
                ...
                for data_item in input_data:
                    ...
                    if self.use_barcode_discriminator:
                        loss = self.adaptive_loss(true_scores, fake_scores)
                    else:
    ...
    def save_segmentation_images(self, data, segmentations, scores, save_path='./output'):
        image_paths = [
        ...
        if isinstance(scores, np.ndarray):
            scores = scores.tolist() if scores.ndim == 1 else [scores[i] for i in range(len(scores))]
