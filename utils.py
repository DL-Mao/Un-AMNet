...
def plot_segmentation_images(
    ...
    masks_provided = any(mp not in (None, "-1") for mp in mask_paths)
    ...
    for image_path, mask_path, anomaly_score, segmentation in tqdm.tqdm(
        ...
        path_norm = os.path.normpath(image_path)
        path_parts = path_norm.split(os.sep)
        save_parts = path_parts[-save_depth:] if save_depth > 0 else [path_parts[-1]]
        savename = "_".join(save_parts)
        savename = os.path.join(savefolder, savename)

        num_panels = 3 if mask_available else 2
        f, axes = plt.subplots(1, num_panels)
        if num_panels == 1:
            axes = [axes]
        ...
        if mask_available:
            ...
            axes[1].axis("off")
        f.set_size_inches(3 * num_panels, 3)
        f.tight_layout()
        f.savefig(savename)
        plt.close()
